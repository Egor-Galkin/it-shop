...
async function main() {
  ...
  try {
    ...
    await pool.query(`
      ...
      (2, 2, 13, 4, false, 'Мощный ноутбук, но тяжёлый. Для работы — идеально.', '${now}'),
      (3, 2, 24, 5, false, 'Звук чистый, шумоподавление работает отлично. Рекомендую!', '${now}'),
      (4, 2, 28, 3, false, 'Часы хорошие, но цена завышена. Батарея слабовата.', '${now}'),
      (5, 3, 5, 5, false, 'Galaxy S24 Ultra — монстр! Экран, камера, производительность — всё на высоте.', '${now}'),
      (6, 3, 20, 4, false, 'iPad Pro отличный, но для простых задач избыточен.', '${now}'),
      (7, 3, 25, 5, false, 'Sony WH-1000XM5 — лучшие наушники для путешествий. Тишина полная!', '${now}'),
      (8, 4, 2, 4, false, 'iPhone 15 — надёжный, но без особых инноваций.', '${now}'),
      (9, 4, 6, 3, false, 'Galaxy S24 хорош, но перегревается в играх.', '${now}'),
      (10, 4, 14, 5, false, 'MacBook Air M2 — лёгкий, быстрый, батарея на весь день. Идеален для учёбы!', '${now}'),
      (11, 4, 26, 4, false, 'Buds2 Pro удобные, звук хороший, но кейс скользкий.', '${now}'),
      (12, 4, 31, 2, false, 'iMac красивый, но за эти деньги ожидал больше производительности.', '${now}'),
      (13, 5, 9, 5, false, 'Xiaomi 14 Pro — флагман за разумные деньги. Камера впечатляет!', '${now}'),
      (14, 5, 16, 4, false, 'ASUS ROG Zephyrus — игровой зверь. Шумный, но мощный.', '${now}'),
      (15, 6, 11, 5, false, 'Redmi Note 13 Pro — топ за свои деньги! Экран сочный, камера достойная.', '${now}'),
      (16, 6, 22, 4, false, 'Galaxy Tab S9 отличный планшет, но дорогой.', '${now}'),
      (17, 6, 30, 3, false, 'Xiaomi Watch S3 нормальные, но функционал ограничен по сравнению с Apple.', '${now}'),
      (45, 10, 24, 3, true, 'example example examples asd', '${now}')
      ON CONFLICT (id) DO NOTHING;
    `);
